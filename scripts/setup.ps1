...
function Get-VibeshineGamepadInstaller {
    param([Parameter(Mandatory = $true)][string]$SearchRoot)
...
$gamepadPayloadPresent = [bool](Get-VibeshineGamepadInstaller -SearchRoot $vibeshineRoot)
$needsUpdate = (-not $vibeshineExe) -or ($installedTag -ne $latestTag) -or (-not $gamepadPayloadPresent)

if ($vibeshineExe -and $installedTag -eq $latestTag -and -not $gamepadPayloadPresent) {
    Write-Host 'Current Vibeshine payload is missing the bundled VHF gamepad installer; refreshing the same release.'
}
...
if ($needsUpdate) {
    ...
    try {
        ...
        $adminArgs = '/a /qn TARGETDIR="{0}" INSTALL_VIRTUAL_DISPLAY_DRIVER=0 INSTALL_VIRTUAL_GAMEPAD_DRIVER=1' -f $stageRoot
...
$gamepadInstaller = Get-VibeshineGamepadInstaller -SearchRoot $vibeshineRoot
if ($gamepadInstaller -and (Test-Path -LiteralPath $gamepadInstaller)) {
    ...
    Write-Warning 'Vibeshine VHF gamepad installer is still missing after refresh. If your normal Vibeshine installation already provides controller input, you can continue; otherwise controller emulation may be unavailable.'
}
